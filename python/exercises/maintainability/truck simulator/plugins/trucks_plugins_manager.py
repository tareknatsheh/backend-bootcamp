...
_all_truck_plugins_names = [f.replace(".py","") for f in _all_files_in_truck_folder if f.endswith('.py') and not (f.startswith("__") or f == "Main.py")]
_all_truck_modules_list = [import_module(f"plugins.trucks.{truck_module_name}").truck for truck_module_name in _all_truck_plugins_names]
