...
def test_shows_list_visited_places(good_player):
    data = good_player.show_some_places_and_weapon()
...
def test_shows_one_fav_weapon(good_player):
    data = good_player.show_some_places_and_weapon()
...
def test_fail_if_visited_places_empty():
    with pytest.raises(Exception):
        Player("test player", False, [], weapons) 

def test_fail_if_fav_weapons_empty():
    with pytest.raises(Exception):
        Player("test player", False, visited_places, []) 

def test_fail_if_name_empty():
    with pytest.raises(Exception):
