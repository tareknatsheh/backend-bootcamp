# We want to decipher a secret message given as a list of key numbers
# each number is the word count in reference text.
# -- Note: in the given example you started counting words from 1, but here, and after multiple tries
# -- I've found that the counting starts from 0

# we take each relevant word and return the first character of it

def decipher(keys, text):
    secret_msg = ""
    text_array = text.split()
    for key in keys:
        secret_msg += text_array[key][0]
...
deciphered_msg = decipher(keys, text)
